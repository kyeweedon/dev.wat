	...
	*/
	...
	function makeCausation($job, $taskID) {
		...
		if(count($buildingImages) > 0) {
			...
			$pdf->addJpegFromFile($buildingImages[0]['imageThumb'], 45, $cursorY-=160, '', 150);
		...
		$pdf->addText(365, $cursorY, 11, $sMob);
		...
		$pdf->addText(365, $cursorY-=15, 11, $sPhone);
		// }
		...
		// Build Content
		list($pdf, $pageCount, $cursorY) = addNewPage($pdf, $pageCount, $pageTitle);
		buildContent($pdf, $pageCount, $pageTitle, $cursorY, 'causation', $buildings, $locations, $elements);
	...
	function addNewPage($pdf, $pageCount, $pageTitle) {
		...
		$pdf->selectFont('./fonts/Helvetica.afm');
		$copy = utf8_decode("©");
		$pdf->addText(343, 55, 8, $copy." 2011 Watson Inc.");	
		$pdf->setColor(0.23,0.22,0.22);
	...
	function addLocation($pdf, $cursorY, $location) {
		...
		$pdf->setColor(0.87,0.87,0.85);
		$pdf->filledRectangle(35, $cursorY-=30,525,40);
		$pdf->setColor(0.23,0.22,0.22);
		$pdf->selectFont('./fonts/HelveticaNeue-CondensedBlack.afm');
		$pdf->addText(45, $cursorY+=15, 15, $location['name']);
		
		// Dimensions
		$pdf->selectFont('./fonts/Helvetica.afm');
		$pdf->addText(440, $cursorY+=1, 10, $location['length']." x ".$location['width']." x ".$location['height']."mm");	
	...
	function addScopeElement($pdf, $cursorY, $element) {
		...
		$pdf->addText(45, $cursorY-=20, 12, $element['name'] . ' ' . $element['attribute']);
	
		// Paint Qty
		if ($element['paintQty'] != '0') {
			$pdf->selectFont('./fonts/Helvetica.afm');
			$pdf->addText(370, $cursorY, 12, "Paint - ".$element['paintQty']." L");
		}
		...
		$pdf->addText(45, $cursorY-=15, 12, $action);
	...
	function addCausationElement($pdf, $cursorY, $element) {
		...
		$pdf->line(35,$cursorY,560,$cursorY);
		$pdf->selectFont('./fonts/Helvetica-Bold.afm');
		$pdf->addText(45, $cursorY-=20, 12, $element['name']);
	
		// Damage
		$pdf->selectFont('./fonts/Helvetica.afm');
		$pdf->addText(370, $cursorY, 12, $element['damage']);
		
		// Event
		$right=$pdf->getTextWidth(12, $element['event']);
		$pdf->addText(550-$right, $cursorY, 12, $element['event']);
		
		// Notes
		$pdf->setColor(0.6,0.6,0.6);
	
		if ($element['notes'] != NULL) {
		$pdf->addText(45, $cursorY-=15, 12, $element['notes']);
		}
		$cursorY -= 13;
	...
	function addBuildingImage($pdf, $cursorY, $bdimage, $bdname, $rowassist) {
		...
		if ($rowassist == 1) {
			...
			$pdf->selectFont('./fonts/Helvetica-Bold.afm');
			$pdf->addJpegFromFile($bdimage, 45, $cursorY, '', 190);
			...
			$pdf->selectFont('./fonts/Helvetica-Bold.afm');
			$pdf->addJpegFromFile($bdimage, 300, $cursorY, '', 190);
